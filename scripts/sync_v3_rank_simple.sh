...
# ========================================
# v3_rank_daily 단순 동기화 스크립트
# v2_slot_tasks_daily_progress 테이블 사용
# min_rank 우선, 0이 아닌 가장 높은 순위 선택
# ========================================
...
DEBUG=${DEBUG:-false}  # DEBUG=true ./sync_v3_rank_simple.sh 형태로 실행 가능
TEMP_DIR="/tmp/v3_rank_sync_simple_$$"

# 로그 디렉토리 설정
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"
LOG_DIR="$PROJECT_DIR/logs"
LOG_FILE="$LOG_DIR/sync_v3_simple_$(date +%Y%m%d_%H%M%S).log"
...
# ========================================
# 2. 외부 DB에서 정보 동기화 (단순 로직)
# ========================================
sync_from_external() {
    log_info "외부 DB에서 정보 동기화 시작 (단순 모드)..."
...
EOF
        ...
        # 외부 DB에서 순위 정보 가져오기 (min_rank 우선, rank_data 보조)
        # v2_slot_tasks_daily_progress 테이블 사용
        ...
        SELECT 
            CASE 
                -- min_rank가 있고 500 이하면 사용
                WHEN min_rank IS NOT NULL AND min_rank > 0 AND min_rank <= 500 THEN min_rank
                -- min_rank가 500 초과면 0
                WHEN min_rank IS NOT NULL AND min_rank > 500 THEN 0
                -- min_rank가 NULL/빈값이고 rank_data에 0이 있으면 0
                WHEN (min_rank IS NULL OR min_rank = 0) AND has_zero_rank THEN 0
                -- 그 외는 NULL (측정중)
                ELSE NULL
...
EOF
    ...
    log_info "========================================="
    log_info "동기화 완료 - $CHECK_DATE (단순 모드)"
    log_info "========================================="
...
main() {
    log_info "========================================="
    log_info "v3_rank_daily 단순 동기화 시작"
    log_info "대상 날짜: $CHECK_DATE"
    log_info "외부 테이블: v2_slot_tasks_daily_progress"
    log_info "로직: min_rank 사용 (500 초과 시 0, NULL은 측정중)"
    log_info "========================================="
